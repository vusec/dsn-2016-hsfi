...
static void hypermem_write(const struct hypermem_session *session,
	hypermem_entry_t value) {
	memcpy_toio(session->address, &value, sizeof(value));
}
...
int hypermem_connect(struct hypermem_session *session)
{
	...
	/* We need to remap the VGA memory inside the kernel virtual 
	 * space in order to access it */
	kvga = ioremap(HYPERMEM_BASEADDR, HYPERMEM_SIZE);
	if (kvga == NULL) {
		printk("<EDFI>: Cannot remap VGA memory\n");
		goto connect_failed;
	}
	session->address = session->mem_base = kvga;
	channel = (void __iomem *)hypermem_read(session);
	if ((unsigned long)channel < HYPERMEM_BASEADDR ||
		(unsigned long)channel >= (HYPERMEM_BASEADDR + HYPERMEM_SIZE)) {
		printk("<EDFI>: Hypermem connect failed. Channel %p out of range\n", channel);
		iounmap (kvga);
		goto connect_failed;
	}
	session->address = phys_to_virt((phys_addr_t)channel);
...
asmlinkage void onfault_hypermem(int bb_index)
{
...
void hypermem_report_startup()
{
	struct hypermem_session session;

	if (hypermem_connect(&session) < 0) {
		printk("<EDFI-HYPERMEM>: Failed connect\n");
		return;
	}
	hypermem_print(&session, "<EDFI> startup vmlinux\n");
	hypermem_set_cr3(&session, read_cr3());
	hypermem_edfi_context_set(&session, edfi_module_name, edfi_context, 0);
	edfi_context->c.faulty_bb_index = hypermem_edfi_faultindex_get(&session, edfi_module_name);
	hypermem_edfi_dump_stats_module(&session, edfi_module_name, "hypermem_report_startup");

	edfi_onfault_p = onfault_hypermem;
